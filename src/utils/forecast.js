...
const forecast = (latitude, longitude, callback) => {
    ...
    request({ url: url, json: true }, (error, response) => {
        if(error) {
            ...
            callback(undefined, `${response.body.current.weather_descriptions[0]}. It is currently ${response.body.current.temperature} degrees out. There is a ${response.body.current.precip * 100}% chance of rain. The humidity is ${response.body.current.humidity}%`);
        }
    })
};
